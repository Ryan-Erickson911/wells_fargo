...
class _PlaceMapState extends State<PlaceMap> {
  ...
  Future<Marker> _createPlaceMarker(
    ...
    final marker = Marker(
      ...
      infoWindow: InfoWindow(
        title: place.name,
        snippet: '${place.invst_value} Current Value (added to account)',
    ...
    setState(() {
      final updatedMarker = marker.copyWith(
        infoWindowParam: InfoWindow(
          title: place.name,
          snippet:
              place.invst_value != 0 ? '${place.invst_value} Current Investment Value' : null,
        ),
      );
  ...
  Future<BitmapDescriptor> _getPlaceMarkerIcon(PlaceCategory category) =>
      switch (category) {
        PlaceCategory.neutral => BitmapDescriptor.fromAssetImage(
            createLocalImageConfiguration(context, size: const Size.square(32)),
            'assets/images/2.0x/heart.png'),
        PlaceCategory.more => BitmapDescriptor.fromAssetImage(
            createLocalImageConfiguration(context, size: const Size.square(32)),
            'assets/images/2.0x/flutter_logo.png'),
        PlaceCategory.less => Future.value(BitmapDescriptor.defaultMarker),
      };
            ...
            children: <Widget>[
              FilledButton(
                style: FilledButton.styleFrom(
                    backgroundColor:
                        selectedPlaceCategory == PlaceCategory.neutral
                            ? Colors.green[700]
                            : Colors.lightGreen),
                onPressed: () => onChanged(PlaceCategory.neutral),
                child: const Text(
                  'Income',
              ...
              FilledButton(
                style: FilledButton.styleFrom(
                    backgroundColor:
                        selectedPlaceCategory == PlaceCategory.more
                            ? Colors.green[700]
                            : Colors.lightGreen),
                onPressed: () => onChanged(PlaceCategory.more),
                child: const Text(
                  'Invest More!',
                  style: TextStyle(color: Colors.white, fontSize: 14.0),
                ),
              ...
              FilledButton(
                style: FilledButton.styleFrom(
                    backgroundColor:
                        selectedPlaceCategory == PlaceCategory.less
                            ? Colors.green[700]
                            : Colors.lightGreen),
                onPressed: () => onChanged(PlaceCategory.less),
                child: const Text(
                  'Pending Changes',
                  style: TextStyle(color: Colors.white, fontSize: 14.0),
                ),
